...
import { Hash, MessageCircle, Ghost, ChevronLeft, ChevronRight, LogOut, Plus, ChevronDown, ChevronUp } from 'lucide-react';
...
export function Sidebar({ college, currentFilter, onFilterChange, onSignOut, username }: SidebarProps) {
    ...
    const handleCreateGroup = async (e: React.FormEvent) => {
        ...
        // Check if user has created a group in the last 24h
        // Note: This requires the 'created_by' column in hashtag_groups
        // If the column doesn't exist yet, this check might fail or be skipped depending on RLS/Schema
        // We'll try-catch it or assume it works if schema is updated.
        ...
        try {
            ...
            // Proceeding if check fails to avoid blocking during migration
        }
    ...
    return (
        <div
            className={cn(
                "bg-slate-900/50 backdrop-blur-md border-r border-white/10 flex flex-col h-full transition-all duration-300 ease-in-out relative z-20",
        ...
        >
            ...
            <button
                ...
                className="absolute -right-3 top-6 bg-slate-800 border border-white/10 rounded-full p-1 text-gray-400 hover:text-white hover:bg-slate-700 transition-colors z-50"
            >
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide">
                ...
                <div className="p-2 space-y-1">
                    ...
                    <button
                        onClick={() => onFilterChange('confession')}
                        className={cn(
                            "w-full flex items-center transition-all duration-200 group relative",
                            isCollapsed ? "justify-center p-2 rounded-lg" : "space-x-3 px-3 py-2 rounded-lg",
                ...
                <div className={cn("px-2 pb-2", isCollapsed && "flex flex-col items-center")}>
                    ...
                    <div className="space-y-0.5 w-full">
                        {displayedGroups.map((group, index) => (
                            <button
                                key={group.id}
                                onClick={() => onFilterChange(`#${group.hashtag}`)}
                                className={cn(
